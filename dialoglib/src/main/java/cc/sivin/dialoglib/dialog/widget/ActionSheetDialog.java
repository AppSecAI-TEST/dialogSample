package cc.sivin.dialoglib.dialog.widget;
...
public class ActionSheetDialog extends BottomBaseDialog {
    ...
    /**
     * item bottom textcolor
     */
    private int itemTextBottomColor = Color.parseColor("#FE5864");
    /**
    ...
    public ActionSheetDialog(Context context, String[] items, View animateView) {
        ...
        for (String item : items) {
            DialogMenuItem customBaseItem = new DialogMenuItem(item,itemTextColor, 0);
    ...
    public void setUiBeforShow() {
        ...
        tv_cancel.setTextColor(itemTextBottomColor);
    ...
    public void updateText(String text, int position) {
        contents.get(position).operName = text;
        if (adapter != null)
            adapter.notifyDataSetChanged();
    }

    public void upDateText(int color ,int position){
        contents.get(position).textColor = color;
        if (adapter != null)
            adapter.notifyDataSetChanged();
    }
